...
func coverWithThermopasta():
	if not isCoveredWithThermopasta:
		...
		WorldSignals.emit_signal('notify', 'Новый процессор покрыт термопастой')

func freeze_toggle():
	isGrabbed = false
	freeze = true
	WorldSignals.emit_signal('notify', 'Новый процессор заморожен')
...
func pin_to_motherboard():
	...
	if Motherboard.place_cpu_obj == null:
		...
		freeze = true
		WorldSignals.emit_signal('notify', 'Новый процессор вставлен')
...
func unpin_from_motherboard():
	...
	if Motherboard.place_cpu_obj == self:
		...
		freeze = false
		WorldSignals.emit_signal('notify', 'Новый процессор вытащен')
