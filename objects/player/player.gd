...
func _process(delta):
	Player.grab_point = $Head/Camera/hand.global_position
...
func _unhandled_input(event):
	...
	if event.is_action_pressed('e'):
		if raycast_hand.is_colliding() and not raycast_hand.get_collider() == null:
			if raycast_hand.get_collider().has_method('grab'):
				raycast_hand.get_collider().grab()
	if event.is_action('c'):
		if raycast_hand.is_colliding() and not raycast_hand.get_collider() == null:
			if raycast_hand.get_collider().has_method('rotate_left'):
				raycast_hand.get_collider().rotate_left()
	if event.is_action('v'):
		if raycast_hand.is_colliding() and not raycast_hand.get_collider() == null:
			if raycast_hand.get_collider().has_method('rotate_right'):
				raycast_hand.get_collider().rotate_right()
	if event.is_action_pressed('x'):
		if raycast_hand.is_colliding() and not raycast_hand.get_collider() == null:
			if raycast_hand.get_collider().has_method('pin_unpin'):
				raycast_hand.get_collider().pin_unpin()
